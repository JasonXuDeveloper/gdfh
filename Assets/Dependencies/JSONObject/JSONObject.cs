...
public class JSONObject {
	...
	public void AddField(string name, object val) {
		if (val is bool)
		{
			AddField(name, Create((bool)val));
		}
		else if (val is float)
		{
			AddField(name, Create((float)val));
		}
		else if (val is int)
		{
			AddField(name, Create((int)val));
		}
		else if (val is string)
		{
			AddField(name, Create((string)val));
		}
	}
	...
	public bool HasField(string name,out object val) {
		if(type == Type.OBJECT)
			for(int i = 0; i < keys.Count; i++)
				if (keys[i] == name)
				{
